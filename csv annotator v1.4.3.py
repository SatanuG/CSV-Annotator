...
label1 = tk.Label(frame1, text='Enter the Tweet ID (starts from index 0)')
...
def terminate():
    ...
    messagebox.showinfo("TweetID", "Closing tweet ID is: "+str(tweet_counter-1))
    sys.exit()
...
def next():
    ...
    if emotion_type is None:
        messagebox.showerror("Alert!!", "No input given!! Choose a radio button")
    else:
...
def submit():
    ...
    if int(rowNum) == tweet_counter:
        ...
        df_to_write.to_csv("annotated_csv_"+rowNum+"-"+str(tweet_counter-1)+".csv", index=True, encoding="utf-8")
